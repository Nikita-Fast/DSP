...
class QAMDemodulator(BlockDemodulator):
    ...
    def demodulate_soft(self, symbols, noise_variance):
        ...
        for r in symbols:
            ...
            #     # используем выведенную формулу
            #     if self.bits_per_symbol == 2:
            #         llr_b0 = 2 * np.real(r) / noise_variance
            #         llr_b1 = -2 * np.imag(r) / noise_variance
            #         llr_list[count] = llr_b1
            #         llr_list[count + 1] = llr_b0
            #         count = count + 2
            #
            #     elif self.bits_per_symbol == 4:
            #         var = -1 / (2 * noise_variance)
            #         re = np.real(r)
            #         shifts = np.array([-3, -1, 1, 3])
            #         e = np.exp(var * ((re + shifts) ** 2))
            #         llr_b0 = np.log((e[0] + e[2]) / (e[1] + e[3]))
            ...
            #         count = count + 1
            #     else:
            #         raise ValueError("no LLR formula for QAM of order=%i" % (len(self.constellation)))
            # else:
