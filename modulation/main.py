...
def plot_ber_curve(input_bits, order):
    t = time.time()
    BERs = []

    qam_modulator = modulation.QAMModulator(order=order)
    awgn_channel = channel.AWGNChannel()
    qam_demodulator = demodulation.QAMDemodulator(order=order)

    mod_signal = qam_modulator.modulate_using_matrix(input_bits)
    ...
    plt.xlabel("Eb/N0, dB")
    plt.ylabel("BER")
    plt.plot(np.linspace(start=0, stop=max_Eb_N0_dB-1, num=max_Eb_N0_dB), BERs, '--bo', label=str(order)+'-QAM')
    plt.legend()
    plt.show()

order=16
bits = gen_bits(0, int(symbols_num * np.log2(order)))
plot_ber_curve(bits, order)
